...
motif_file = "D:/Research/pepsickle-paper/data/model_weights/all_mammal_20S_digestion_motif_mod.pt"
sequence_file = "D:/Research/pepsickle-paper/data/model_weights/all_mammal_20S_digestion_sequence_mod.pt"
...
sequence_model.load_state_dict(torch.load(sequence_file))
# reset final layer
sequence_model.out = nn.Linear(sequence_model.out.in_features, 2)

motif_model = MotifNet()
motif_model.load_state_dict(torch.load(motif_file))
# reset final layer
motif_model.out = nn.Linear(motif_model.out.in_features, 2)
...
seq_optimizer = optim.Adam(sequence_model.parameters(), lr=.0005)

motif_criterion = nn.NLLLoss(
    weight=torch.tensor([1, len(neg_train)/len(pos_train)]).type(dtype))
motif_optimizer = optim.Adam(motif_model.parameters(), lr=.0005)
